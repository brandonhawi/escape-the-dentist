...
function splitBsp(node, rng, depth) {
    ...
    let splitH;
    if (node.w > node.h * 1.25 && canSplitV)
        splitH = false;
    else if (node.h > node.w * 1.25 && canSplitH)
        splitH = true;
    else
        splitH = canSplitH && (!canSplitV || rng() < 0.5);
    if (splitH) {
...
function placeRooms(node, m, rng) {
    if (node.left || node.right) {
        if (node.left)
            placeRooms(node.left, m, rng);
        if (node.right)
            placeRooms(node.right, m, rng);
        return;
    }
    // leaf
...
function connectSiblings(node, m, rng) {
    if (!node.left || !node.right)
        return;
...
function bfsCanReach(m, sx, sy, tx, ty) {
    const seen = new Set();
    const q = [[sx, sy]];
    seen.add(sx + ',' + sy);
    while (q.length) {
...
export function generateFloor(level) {
    ...
    for (let x = exitRoom.x + exitRoom.w; x < COLS - 1; x++)
        m[exitY][x] = T_FLOOR;
